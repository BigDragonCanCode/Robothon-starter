...
def apply_motion_frame(
    ...
    wave = math.sin(2.0 * math.pi * 0.9 * elapsed_s)
    ...
    if intent in {"walk_forward", "step_back"}:
        ...
        base_z += 0.005 * math.sin(gait) + 0.01 * max(0.0, stance_bias)
        base_y += 0.018 * stance_bias
        ...
        pose["left_shoulder_pitch_joint"] = 0.03 - arm_swing
        pose["right_shoulder_pitch_joint"] = 0.03 + arm_swing
        pose["left_shoulder_roll_joint"] = 0.1 + 0.02 * math.sin(gait + math.pi / 2.0)
        pose["right_shoulder_roll_joint"] = -0.1 - 0.02 * math.sin(gait + math.pi / 2.0)
        pose["left_elbow_joint"] = -0.5 - 0.08 * math.sin(gait + math.pi / 2.0)
        pose["right_elbow_joint"] = -0.5 + 0.08 * math.sin(gait + math.pi / 2.0)
    elif intent in {"turn_left", "turn_right"}:
        turn_sign = 1.0 if intent == "turn_left" else -1.0
        turn_cycle = 2.0 * math.pi * progress
        base_z += 0.004 * math.sin(turn_cycle) + 0.006 * max(0.0, math.sin(turn_cycle + math.pi / 2.0))
        base_y += 0.012 * turn_sign * math.sin(turn_cycle)
        pose["waist_yaw_joint"] = 0.14 * turn_sign * math.sin(math.pi * progress)
        pose["waist_roll_joint"] = 0.04 * turn_sign * math.sin(turn_cycle + math.pi / 2.0)
        pose["head_yaw_joint"] = 0.16 * turn_sign * math.sin(math.pi * progress)
        _apply_turn_step_gait(pose, turn_cycle, turn_sign)
        arm_turn_swing = 0.12 * math.sin(turn_cycle)
        pose["left_shoulder_pitch_joint"] = 0.03 - arm_turn_swing
        pose["right_shoulder_pitch_joint"] = 0.03 + arm_turn_swing
        pose["left_shoulder_roll_joint"] = 0.11 + 0.03 * turn_sign
        pose["right_shoulder_roll_joint"] = -0.11 + 0.03 * turn_sign
        pose["left_elbow_joint"] = -0.5 - 0.06 * math.sin(turn_cycle + math.pi / 2.0)
        pose["right_elbow_joint"] = -0.5 + 0.06 * math.sin(turn_cycle + math.pi / 2.0)
    elif intent == "wave":
        raise_progress = smoothstep(0.0, duration_s * 0.35, elapsed_s)
        wave_progress = smoothstep(duration_s * 0.28, duration_s * 0.55, elapsed_s)
        pose["head_yaw_joint"] = 0.12 * raise_progress + 0.1 * wave_progress * math.sin(0.7 * gait)
        pose["head_pitch_joint"] = -0.02 + 0.03 * raise_progress
        pose["left_shoulder_pitch_joint"] = 0.1
        pose["left_shoulder_roll_joint"] = 0.16
        pose["left_elbow_joint"] = -0.46

        pose["right_shoulder_pitch_joint"] = lerp(0.08, -0.18, raise_progress) + 0.06 * wave_progress * wave
        pose["right_shoulder_roll_joint"] = lerp(-0.18, -0.58, raise_progress) + 0.08 * wave_progress * math.sin(1.5 * gait)
        pose["right_shoulder_yaw_joint"] = lerp(0.08, 0.22, raise_progress)
        pose["right_elbow_joint"] = lerp(-0.42, -1.0, raise_progress) + 0.16 * wave_progress * math.sin(1.8 * gait)
        pose["right_wrist_yaw_joint"] = 0.42 * wave_progress * math.sin(2.1 * gait)
        pose["right_wrist_roll_joint"] = 0.14 * wave_progress * math.sin(1.7 * gait)
    elif intent == "bow":
        bow_amount = 0.42 * math.sin(math.pi * progress)
        base_z -= 0.05 * bow_amount
        pose["waist_pitch_joint"] = 0.03 + bow_amount
        pose["head_pitch_joint"] = -0.18 * bow_amount
        pose["left_shoulder_pitch_joint"] = 0.10
        pose["right_shoulder_pitch_joint"] = 0.10
        pose["left_elbow_joint"] = -0.62
        pose["right_elbow_joint"] = -0.62
    elif intent in {"stop", "idle"}:
        if intent == "idle":
            base_z += 0.0025 * math.sin(0.45 * gait)
            pose["waist_pitch_joint"] = 0.06 + 0.01 * math.sin(0.25 * gait)
            pose["head_yaw_joint"] = 0.025 * math.sin(0.35 * gait)
            pose["head_pitch_joint"] = -0.03 + 0.015 * math.sin(0.25 * gait + 0.7)
            pose["left_shoulder_pitch_joint"] = 0.08 + 0.015 * math.sin(0.3 * gait + 0.2)
            pose["right_shoulder_pitch_joint"] = 0.08 - 0.015 * math.sin(0.3 * gait + 0.2)
            pose["left_elbow_joint"] = -0.42 - 0.015 * math.sin(0.28 * gait)
            pose["right_elbow_joint"] = -0.42 + 0.015 * math.sin(0.28 * gait)
        else:
            pose["head_yaw_joint"] = 0.0
    else:
    ...
    if source_pose is not None:
        blend_alpha = smoothstep(0.0, min(0.22, 1.0), progress)
        pose = blend_joint_poses(source_pose, pose, blend_alpha)
...
def _apply_leg_gait(pose: dict[str, float], gait: float, stride_sign: float) -> None:
    ...
    pose["left_hip_pitch_joint"] += 0.08 * stride_sign * left_stance - 0.2 * stride_sign * left_swing
    pose["left_knee_joint"] += 0.05 * left_stance + 0.22 * left_swing
    pose["left_ankle_pitch_joint"] -= 0.04 * stride_sign * left_stance - 0.14 * stride_sign * left_swing
    pose["left_ankle_roll_joint"] += 0.02 * math.sin(gait + math.pi / 2.0)

    pose["right_hip_pitch_joint"] += 0.08 * stride_sign * right_stance - 0.2 * stride_sign * right_swing
    pose["right_knee_joint"] += 0.05 * right_stance + 0.22 * right_swing
    pose["right_ankle_pitch_joint"] -= 0.04 * stride_sign * right_stance - 0.14 * stride_sign * right_swing
    pose["right_ankle_roll_joint"] -= 0.02 * math.sin(gait + math.pi / 2.0)
